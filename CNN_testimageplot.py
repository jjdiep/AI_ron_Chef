        ...
        for img_name in imgs:
            ...
            try:
                ...
                if w < min_side:
                    ...
                    img_arr_rs = imresize(img_arr, (min_side, hsize, z))
                    resize_count += 1
                elif h < min_side:
                    hpercent = (min_side/float(h))
                    wsize = int((float(w)*float(hpercent)))
                    #print('new dims:', wsize, min_side)
                    img_arr_rs = imresize(img_arr, (wsize, min_side, z))
                    resize_count += 1
...
if __name__ == "__main__":
    ...
    class_to_ix = {}
    ix_to_class = {}
    with open('segmentedimages/classes.txt', 'r') as txt:
        classes = [l.strip() for l in txt.readlines()]
        class_to_ix = dict(zip(classes, range(len(classes))))
        ix_to_class = dict(zip(range(len(classes)), classes))
        class_to_ix = {v: k for k, v in ix_to_class.items()}

    with open('data-origin/classes.txt') as txt:
        classes1 = [l.strip() for l in txt.readlines()]
    ...
    '''X_train, y_train = load_images('data-origin/trainimage', min_side=299)

    pdb.set_trace()'''

    X_test, y_test = load_images('segmentedimages/testimage', min_side=299)
    pdb.set_trace()
    ...
    for i in range(len(y_test)):
        pred[i]= preds_top_1[i][0][0]
        print(pred)




    root_dir = 'segmentedimages/testimage/'
    rows = 1
    cols = 3
    ...
    for i in range(rows):
         for j in range(cols):
            all_files = os.listdir(os.path.join(root_dir, sorted_food_dirs[0]))
            img = plt.imread(os.path.join(root_dir,sorted_food_dirs[0], all_files[j]))
            ax[j].imshow(img)
            ec = (0, .6, .1)
            fc = (0, .7, .2)
            predi = pred[j][0]
            predi = predi.astype(int)
            ax[j].text(0, -20, classes1[predi], size=10, rotation=0,
                    ha="left", va="top", 
